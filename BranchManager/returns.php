...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['order_id'], $_POST['branch_id'], $_POST['staff_id'], $_POST['return_qty'])) {
  header("Location: manager_returns.php");
  exit();
}
...
try {
    ...
    $conn->commit();
    header("Location: manager_returns.php?success=1&order=" . urlencode($order_id));
    exit();

} catch (Exception $e) {
    $conn->rollback();
    header("Location: manager_returns.php?error=" . urlencode($e->getMessage()));
    exit();
}